document.addEventListener('DOMContentLoaded', function () {
  ...
  async function loadCount() {
    ...
    if (!p || !p.auth.isLoggedIn()) {
      statusMsg.textContent = 'Log in to save your count across sessions.'
      return
    }
    try {
      var saves = await p.saves.load('hello-world')
      ...
      statusMsg.textContent = 'Could not load saved count.'
    }
  }
  ...
  async function saveCount() {
    var p = window.Platform
    if (!p || !p.auth.isLoggedIn()) {
      statusMsg.textContent = 'Log in first to save.'
      return
    }
    try {
      await p.saves.save('hello-world', SAVE_LABEL, { count: count })
